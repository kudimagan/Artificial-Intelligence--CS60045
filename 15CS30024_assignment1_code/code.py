...
class EightPuzzle:
    ...
    def solveWithBound(self,h,bound,arr):
        """Helper function for IDA* search for goal state.
        h(puzzle) - heuristic function, returns an integer
        """
        def is_solved(puzzle):
            return puzzle.adj_matrix == _goal_state
        ...
        if fval > bound:
            return fval
        if (is_solved(x)):
            path = x._generate_solution_path([])
            print "No of nodes expanded is equal to " , arr[0]
            return "Goal found"
        succ = x._generate_moves()
        for move in succ:
            arr[0] += 1
            temp = move.solveWithBound(h,bound,arr)
            if temp == "Goal found":
                return "Goal found"
            if temp < minval:
                minval = temp
    ...
    def idaSearch(self,h):
        bound = h(self)
        mov_count = 0
        arr = [mov_count]
        start =  time.time()
        while(True):
            t = self.solveWithBound(h,bound,arr)
            if t == "Goal found":
                end = time.time()
...
def main():
    ...
    while j < 1:
        ...
        print "No of nodes expanded is equal to " , count
        print "Time taken ", end - start
        start=time.time()
        path, count = p.solve(h_manhattan_lsq)
        end= time.time()
        print "Manhattan least squares heuristic\n"
        print "No of nodes expanded is equal to " , count
        print "Time taken ", end - start
        start = time.time()
        ...
        print "Linear least squares heuristic\n"
        print "No of nodes expanded is equal to " , count
        print "Time taken ", end - start
        start = time.time()
        ...
        print "____ ______"
        print "Manhattan heuristic\n"
        p.idaSearch(h_manhattan)
        print "Manhattan least squares heuristic\n"
        p.idaSearch(h_manhattan_lsq)
        print "Linear heuristic\n"
        p.idaSearch(h_linear)
        print "Linear least squares heuristic\n"
        p.idaSearch(h_linear_lsq)
        print "Zero heuristic\n"
        p.idaSearch(h_default)
        j = j + 1
